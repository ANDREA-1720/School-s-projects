/*
    ...
    Classe: 3ESA
    Descrizione: "20260309-3esa-Perciabosco-ricerca-binaria"
*/
...
int main() {
  ...
  int indice/*=0*/, indice2 ,comodo, temp, dati, min, max, cercato;
  bool trovato = false;
  ...
  for(indice = 0; indice < dati; indice++){
    cout << "Il valore in posizione " << indice + 1 << " è " << vetnum[indice] << "." << endl << endl;
  }
  ...
  {
    indice = (min + max) / 2;
    if(vetnum[indice] == cercato){
      trovato = true;
    } else if(vetnum[indice] < cercato){
      min = indice + 1;
    } else {
      max = indice - 1;
    }
  ...
  if(trovato){
    cout << "Il numero " << cercato << " è stato trovato in posizione " << indice + 1 << "." << endl;
  } else {
    cout << "Il numero " << cercato << " non è stato trovato." << endl;
  }
